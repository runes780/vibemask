...
合成 eval 的 96% 召回不能直接代表真实文档。对浙大名单做真实召回审计（用 regex 数原文里的学号）：

- **修前**：学号召回 **83.6%**（1405 个真实学号，漏 231 个）。根因：学号是 10 位数字，**regex 不抓**（只抓手机/身份证/邮箱），只能靠模型，模型漏 16%。
- **修后**：学号召回 **100.0%**（1405/1405 全中），PERSON 1435。

**修复**（`privacy_context`）：按列标签的结构化检测——`HEADER_TYPE_MAP` 把列头（学号/工号/姓名/手机/身份证/地址/URL…）映射到类型，**标签列里的每个值按定义就是该类 PII**，直接成 span（`source=SCHEMA, conf=0.9, reason=structural_column:<header>`），不依赖模型逐个识别。这正实现了"把文档解析好"的威力：表头给了强先验，模型只负责补漏。

- 不影响合成 eval（90.1% 不变，结构化只在 `detect_document` 文件路径生效）。
- 不影响精确率（财务表/公司名单无隐私表头 → 不触发结构化，仍 0 误报）。
- URL 召回也已 100%。
...
| 文件 | 类型 | 真实数 | 检出 | 召回 |
|---|---|---:|---:|---:|
| 浙大名单 | 学号 | 1405 | 1405 | **100%** |
| 浙大名单 | 姓名 | 1405 | 1399 | 99.6% |
| 浙大教师名单 | 姓名 | 67 | 68 | ~100% |
| 学生名单 | 姓名 | 40 | 40 | 100% |
| 国家统计局 | 电话 | 1 | 1 | 100% |
| 浙大教师名单 | URL | 63 | 63 | 100% |

（姓名 ground truth = 姓名 列非空单元格数；学号/电话/URL = regex 计数。）
真实召回在所有类型 × 文件上 **≥99.6%**——结构化列检测把表格 PII 的召回基本拉满。
精确率为定性评估（财务词/教育词/表头 FP 已修，无个人 PII 的文档返回 0），尚未像召回一样严格量化（需负样本标注）。

### 已知局限

- **扫描 PDF 不支持**：仓库里的 PDF 是纯图片（3 页无文本层）→ 抽取 0 字符。文本型 PDF 可处理，扫描件需 OCR（超出范围）。
