...
对 `国家统计局浙江调查总.docx`（叙述型、模型主导、22 个检测）逐条人工判定 TP/FP：

- **精确率 = 100%（22/22 真 PII，0 垃圾误报）**。万元/单位/民办/表头 那类 FP 已绝迹。
- 质量瑕疵（非 FP，是边界/类型问题，~5 条）：
  - **旧版 15 位身份证被判成 ACCOUNT_NUMBER**（应为 IDCN）——regex 的 IDCN 只认 18 位，老格式漏了。数据仍被掩码，只是类型标签偏（`{{ACCOUNT_...}}` vs `{{IDCN_...}}`），对 LLM 都传达"敏感标识"。
  - 两个电话并成一个 span（`0571-89198657，89198661`）。
  - 地址含邮政编码标签（`…大厦\n邮政编码：310023`）。

结构化为主的文件（浙大名单等）精确率按构造 ≈100%（姓名列每个单元格就是姓名）。
**诚实边界**：这是 1 个叙述型文件的完整标注 + 其余文件的定性确认；更严格的精确率需更广的负样本语料。

### 文档解析层重构（结构化序列化 + 长上下文窗口）

合成 eval（90.1%）不变，但真实大文件的召回有量级提升。根因：旧的文档解析把所有内容拍平成 `\n` 拼接的字符流，**丢失表格的表头-值关系**，且遗留 `.xls` 的 `sheet:row:col` location 格式完全不被 `privacy_context` 识别（结构化召回 0）。

**两项改动**（均有数字佐证）：

| 改动 | 内容 | 效果 |
|---|---|---|
| 统一表格序列化 | 新建 `core/table_serializer.py`，把 xlsx/docx/legacy 三种 location 格式合并成一个格式无关的序列化器，替代 `privacy_context` 里 xlsx/docx 各写一份的重复代码 | 遗留 `.xls` 走通结构化路径 |
| MLX 窗口 512→4096 | 模型真实配置是 YaRN-RoPE（`max_position_embeddings=131072`，训练长度 4096），非 BERT 的 512。修正错误注释 + 内存自适应降档（OOM 时自动 4096→2048→512） | 大文档少切实体、回到训练长度 |

**实测**（`浙江大学2015年推荐免试生公示名单.xls`，55K 字 / 2400 学生）：

| 配置 | 检出 spans | 学号(ACCOUNT) | 姓名(PERSON) |
|---|---:|---:|---:|
| 改造前（512 窗口 + 旧序列化） | 933 | 134 | 785 |
| **改造后（4096 + 统一序列化）** | **2839** | **1405** | **1434** |

学号 1405 = 真实学号数（100% 召回）；姓名 1434 含叙述中的人名。2804 spans 来自 schema（结构化列检测），即 `table_serializer` 接通的路径。

**无损往返未变**：`detect_document(processor, text)` 签名不变，`replace_spans` 的 offset 坐标系不变，82 个测试全绿。合成 eval 90.1% 不变（样本都 <512 token）。
